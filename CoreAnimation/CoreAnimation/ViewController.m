...
- (void)viewDidLoad {
    ...
    fallingButton.button.backgroundColor = [UIColor whiteColor];
...
//    accel.delegate = self;
    ...
    if (motion.accelerometerAvailable) {
        motion.accelerometerUpdateInterval = 1.0/60.f;
        [motion startAccelerometerUpdatesToQueue:[NSOperationQueue mainQueue] withHandler:^(CMAccelerometerData * _Nullable accelerometerData, NSError * _Nullable error) {
            NSLog(@"*****%@", accelerometerData);
            space.gravity = cpvmult(cpv(accelerometerData.acceleration.x, -accelerometerData.acceleration.y), 100.0f);
        }];
    }
    _motion = motion;
